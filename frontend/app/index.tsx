...
function FormSheet({ mode, visible, onClose, data, initialSale, initialExpense, initialProduction, initialBuyer, onSave }: {
  ...
  useEffect(() => {
    ...
    setPhone(initialBuyer?.phone ?? ""); setAddress(initialBuyer?.address ?? ""); setCategory(initialExpense?.category ?? data.categories[0]); setItemName(initialExpense?.itemName ?? ""); setItemSearch(initialExpense?.itemName ?? ""); setQuantity(initialExpense?.quantity ?? ""); setUnit(initialExpense?.unit ?? "kg"); setCost(initialExpense ? String(initialExpense.totalCost) : ""); setNewCategory("");
  ...
  return <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
    <KeyboardAvoidingView style={styles.modalBackdrop} behavior={Platform.OS === "ios" ? "padding" : "height"}>
      <View style={styles.sheet}><View style={styles.handle} /><ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled"><Text style={styles.sheetTitle}>{title}</Text>
        {mode !== "buyer" ? <><Text style={styles.fieldLabel}>Tanggal</Text><TextInput value={date} onChangeText={setDate} placeholder="YYYY-MM-DD" placeholderTextColor={colors.muted} style={styles.field} /></> : null}
        {mode === "sale" ? <>
