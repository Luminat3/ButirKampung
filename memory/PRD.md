...
- 2026-09-08: Harga telur dinamis (`eggPrice` di AppData, default Rp 3.000, diubah via PriceSheet dari kartu harga di Beranda; total penjualan lama tidak berubah karena tersimpan per transaksi; form penjualan menampilkan harga/butir aktif). Edit & hapus catatan koleksi telur (ikon di tiap baris riwayat, hapus pakai konfirmasi). Stok telur otomatis (total koleksi − total terjual) tampil di Beranda + tab Produksi; form penjualan menampilkan stok tersedia + peringatan non-blocking jika jumlah melebihi stok.
